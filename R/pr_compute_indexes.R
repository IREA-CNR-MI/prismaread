...
#' @param out_file `character` output path - filenames are created adding a suffix to
#'  the basename of this file (e.g., S:/mypath/myoutfile_NDVI.tif)
...
#' @importFrom raster brick raster blockSize writeStart getValues writeValues writeStop
    ...
    if (names(tot_indexes)[[ind]] %in% names(av_indexes)) {
      ...
      if (!file.exists(out_indfile) | overwrite) {
        ...
        if (max(unlist(diffs)) > 9) {
          warning("The bands in the input file do not allow to compute index: ",
                  names(indexes)[[ind]])
        } else {
          indstring <- indform
      ...
      warning("Index ", names(indexes)[[ind]],
              " is not in the current list of available indexes and will not
            be computed. ")
    }
